...
- `sitemap.xml` — canonical homepage URL for search engines
- `capabilities.pdf` — two-page commissioning overview

Version 0.5 adds explicit indexing metadata, a canonical URL, and a discoverable XML sitemap while preserving the site’s content and visual design.

## Before launch

1. Register the selected domain.
2. Replace `bochesterton.com` in `index.html` if a different domain is used.
3. Replace `research@bochesterton.com` if needed.
4. Verify every external research link.
5. Deploy the folder as a static site.
6. Configure custom-domain email.
7. Send a test message to the public address and reply from it.
8. Review the site on desktop and mobile.
9. Archive/tag the launch version.

## Suggested infrastructure

- Registrar: any reputable registrar with transparent renewal pricing.
- DNS / static hosting: Cloudflare is a good fit; a static site does not need a paid hosting tier.
